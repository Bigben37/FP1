...
Mit
\begin{equation}
\label{eq:gauss:multiplymuE}
\tilde{\sigma} := \frac{\sigma(t_{\text{c}})}{\mu_{\text{n}} E} \qquad \text{und} \qquad 
\tilde{A} := \frac{A(t_{\text{c}})}{\mu_{\text{n}} E}
\end{equation}
erhält man
\begin{equation}
\label{eq:gauss}
c(t) = \tilde{A}
\cdot
\frac{1}{\sqrt{2  \pi  \cdot \tilde{\sigma}^2}}
\cdot
e^{-\frac{1}{2}\left(\frac{t - t_{\text{c}}}{\tilde{\sigma}}\right)^2} \ \, .
\end{equation}
